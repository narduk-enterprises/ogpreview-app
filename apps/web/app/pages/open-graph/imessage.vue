...
<template>
  <LayoutsArticleLayout
    title="iMessage Open Graph Meta Tags & Specs"
    description="The exact Open Graph specifications for Apple's iMessage. Ensure your links generate beautiful rich link previews in iOS and macOS Messages."
    :back-link="{ to: '/', label: 'Back to Tool' }"
    :metadata="{ date: 'Updated December 2025', readTime: '3 min read' }"
  >
    <template #cta>
      <ContentCTA
        title="Test Your iMessage Link Previews"
        description="Preview how your links will appear in iMessage and all other platforms instantly."
    ...
    <ContentCallout variant="info" title="Quick Answer" icon="i-lucide-light-bulb">
      iMessage reliably loads standard Open Graph tags. However, it requires a secure URL (`https`) everywhere and is strict about file size and caching. The recommended image is <strong>1200x630px</strong>.
    </ContentCallout>

    <!-- Specs Dashboard -->
    <div class="my-12">
      <h2 class="text-2xl font-bold text-primary dark:text-white mb-6">iMessage Image Specs</h2>
      <div class="grid sm:grid-cols-2 gap-4">
        <UCard>
          ...
          <p class="text-sm text-muted mt-1">Aspect Ratio: 1.91:1.</p>
        </UCard>
    ...
    <div class="my-12 grid md:grid-cols-2 gap-8">
      <div>
        ...
        <ul class="space-y-3">
          <li class="flex items-start gap-2">
            ...
            <span class="text-primary dark:text-dimmed"><strong>Use HTTPS for all image paths.</strong> iMessage strictly refuses to load preview images served via HTTP.</span>
          </li>
          <li class="flex items-start gap-2">
            <UIcon name="i-lucide-check-circle" class="w-5 h-5 text-muted shrink-0 mt-0.5" />
            <span class="text-primary dark:text-dimmed"><strong>Include the Apple Touch Icon.</strong> Use <code>&lt;link rel="apple-touch-icon" href="..."&gt;</code> to provide a site icon representing the source link domain.</span>
          </li>
          <li class="flex items-start gap-2">
            <UIcon name="i-lucide-check-circle" class="w-5 h-5 text-muted shrink-0 mt-0.5" />
            <span class="text-primary dark:text-dimmed"><strong>Expect caching.</strong> iMessage caches link previews on Apple's servers very intensely. Append a cache-buster query parameter to the link if testing.</span>
          </li>
        </ul>
      </div>
      ...
      <div>
        ...
        <UCard class="bg-muted dark:bg-elevated/10 border-default dark:border-default">
          <h3 class="font-semibold text-primary dark:text-dimmed mb-2">My Image Won't Appear in iMessage</h3>
          <p class="text-sm text-primary dark:text-primary mb-4">
            If your image doesn't appear in iMessage, check if the URL provided in <code>og:image</code> is fully qualified and uses <code>https://</code>. Also, make sure Apple's bot isn't blocked by your web server or firewall.
          </p>
        </UCard>
      </div>
    </div>
