...
<template>
  <UCard :ui="{ body: 'p-3 sm:p-2.5 md:p-3' }">
      ...
      >
        <div class="flex items-center justify-center mb-2.5 sm:mb-2">
          ...
          >
            <UIcon
              :name="item.icon"
              class="w-4 h-4 sm:w-3 sm:h-3 text-muted dark:text-dimmed"
              aria-hidden="true"
            />
        ...
        <div
          :aria-label="`${item.name} preview card`"
          role="region"
        ...
        >
          ...
          <div
            v-if="!hasData && item.id !== 'sponsored'"
            class="w-full h-full flex items-center justify-center min-h-[200px] sm:min-h-[200px]"
          >
          ...
          <div v-else class="w-full h-full overflow-hidden">
            <Suspense>
              ...
              <template #fallback>
                <div
                  class="animate-pulse bg-muted dark:bg-elevated w-full h-40 sm:h-48 rounded-lg"
                  :aria-label="`Loading ${item.name} preview`"
                />
              </template>
            </Suspense>
          </div>
    ...
    <div v-if="hasData" class="mt-4 sm:mt-4 space-y-2.5 sm:space-y-2">
      <UAlert
        v-if="validationResult.isComplete"
        color="success"
        variant="subtle"
        icon="i-lucide-circle-check"
        title="All Required Fields Complete"
        description="Your Open Graph tags are ready for all platforms!"
    ...
    <UModal
      ...
      :aria-labelledby="selectedPlatform ? `${selectedPlatform.id}-modal-title` : undefined"
      :ui="{
        footer: 'justify-end',
    ...
    >
      <template v-if="selectedPlatform" #header>
        <div class="flex items-center gap-2 sm:gap-2.5">
          <UIcon
            ...
            class="w-5 h-5 text-muted dark:text-dimmed"
            aria-hidden="true"
          />
          <h2
            :id="`${selectedPlatform.id}-modal-title`"
      ...
      <template #body>
        <div
          ...
          role="region"
          :aria-label="`Detailed ${selectedPlatform.name} preview`"
      ...
      <template #footer="{ close }">
        <UButton
          ...
          color="neutral"
          size="lg"
          class="min-h-[44px] w-full sm:w-auto font-semibold"
