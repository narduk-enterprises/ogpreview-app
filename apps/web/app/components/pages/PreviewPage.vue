...
// URL History
const { history, addToHistory, removeFromHistory, clearHistory, recentHistory } = useUrlHistory();
...
<template>
  ...
  >
    <div class="max-w-380 mx-auto" role="main">
      ...
      <CacheDebugPanel
        v-if="showDebug"
        :debug-info="cacheDebugInfo"
        data-test="cache-debug-panel"
      ...
      <ClientOnly>
        <template #default>
          ...
          <div v-else class="text-center py-12 sm:py-16 px-4">
            ...
            <p
              class="text-base sm:text-base text-muted dark:text-dimmed max-w-md mx-auto leading-relaxed"
            >
              Enter any website URL above to see how it appears when shared on social media
              platforms.
            </p>
          </div>
        </template>
          ...
          <div class="mb-6 sm:mb-4 mt-4 text-center py-12 sm:py-16 px-4" data-test="preview-area">
            ...
            >
              <UIcon
                ...
                class="w-10 h-10 sm:w-10 sm:h-10 text-dimmed animate-pulse"
              />
            </div>
            <h3 class="text-xl sm:text-lg font-semibold text-primary dark:text-white mb-3 sm:mb-2">
              Loading preview...
