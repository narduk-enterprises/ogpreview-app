...
const isOptimizableImage = computed(() => {
  ...
  if (isRemoteHttpImage.value) {
    try {
      const url = new URL(props.src);
      return OPTIMIZABLE_DOMAINS.some(
...
const onImageError = () => {
  ...
  if (
    !usingProxyFallback.value &&
    props.src &&
    isRemoteHttpImage.value &&
  ...
  if (retryCount.value < maxRetries && props.src) {
    ...
    setTimeout(() => {
      ...
      if (import.meta.client) {
        const imgElement = document.querySelector(
          `img[alt="${props.alt}"], img[src*="${imageSrc.value}"]`
        ) as HTMLImageElement;
        if (imgElement) {
    ...
    console.warn(
      '[PreviewImage] Failed to load image:',
      props.src,
      'Proxy fallback:',
      usingProxyFallback.value
    );
...
onMounted(() => {
  if (imageSrc.value && !imageLoaded.value) {
    ...
    setTimeout(() => {
      if (!imageLoaded.value && !imageError.value && import.meta.client) {
...
<template>
  <div
    v-if="src && !imageError"
    :class="['overflow-hidden bg-muted dark:bg-elevated relative', aspectClass]"
  >
    <Transition name="image-fade" mode="out-in">
      <!-- Use NuxtImg only for internal assets or explicitly allowed domains -->
      <NuxtImg
        v-if="isOptimizableImage"
        :key="`nuxt-${imageSrc}`"
        :src="imageSrc"
        :alt="alt"
    ...
    <ClientOnly>
      <div
        ...
        class="absolute inset-0 flex items-center justify-center bg-muted dark:bg-elevated"
        role="status"
        aria-live="polite"
      ...
      >
        <UIcon
          ...
          class="w-8 h-8 text-dimmed dark:text-muted animate-spin"
          aria-hidden="true"
        />
      </div>
    </ClientOnly>
  ...
  <div
    v-else
    :class="['flex items-center justify-center bg-muted dark:bg-elevated', aspectClass]"
    role="img"
  ...
  >
    <div class="text-center">
      <UIcon
        ...
        class="w-12 h-12 text-dimmed dark:text-muted mb-2"
        aria-hidden="true"
      />
