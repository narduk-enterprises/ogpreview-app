    ...
    <article class="max-w-4xl mx-auto">
      ...
      <div class="mb-6">
        <div v-if="platformIcon" class="mb-6 flex items-center gap-4">
          <div class="w-16 h-16 rounded-2xl bg-linear-to-br from-primary-500 to-primary-500 flex items-center justify-center shadow-lg">
        ...
        <p v-if="description" class="text-xl text-muted dark:text-dimmed mb-6">
          {{ description }}
        </p>
        <div v-if="metadata" class="flex flex-wrap gap-4 text-sm text-muted dark:text-dimmed">
      ...
      <!-- Content -->
      <div class="prose prose-lg dark:prose-invert max-w-none bg-white dark:bg-elevated rounded-2xl px-8 py-10 sm:px-12 sm:py-12 shadow-lg">
        <slot />
      </div>
