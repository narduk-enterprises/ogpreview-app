<template>
  <LayoutsArticleLayout
    ...
    description="The exact Open Graph and Twitter Card specifications for X/Twitter. Copy-paste the required meta tags, image sizes, and best practices for perfect link previews."
    :back-link="{ to: '/', label: 'Back to Tool' }"
    :metadata="{ date: 'Updated December 2025', readTime: '3 min read' }"
  >
    <template #cta>
      <ContentCTA
        title="Test Your Twitter Link Previews"
        description="Preview how your links will appear on Twitter and all other platforms instantly."
    ...
    <ContentCallout variant="info" title="Quick Answer" icon="i-heroicons-light-bulb">
      Twitter prefers a <strong>1200×600px</strong> (2:1 ratio) image, although the standard 1200x630px OG tag image usually scales fine. You MUST include <code>twitter:card</code> set to <code>summary_large_image</code> to get the big image preview.
    </ContentCallout>

    <!-- Specs Dashboard -->
    <div class="my-12">
      <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">Twitter Image Specs</h2>
      <div class="grid sm:grid-cols-2 gap-4">
        <UCard>
          <div class="flex items-center gap-3 mb-2">
            <UIcon name="i-heroicons-photo" class="w-6 h-6 text-sky-500" />
            <h3 class="font-semibold text-gray-900 dark:text-white">Recommended Size</h3>
          </div>
          <p class="text-2xl font-bold text-gray-900 dark:text-white">
            1200 × 600px
          </p>
          <p class="text-sm text-gray-500 mt-1">2:1 Aspect Ratio (or 1.91:1)</p>
        </UCard>
        
        <UCard>
          <div class="flex items-center gap-3 mb-2">
            <UIcon name="i-heroicons-document-arrow-down" class="w-6 h-6 text-sky-500" />
    ...
    <div class="my-12 grid md:grid-cols-2 gap-8">
      <div>
        ...
        <ul class="space-y-3">
          <li class="flex items-start gap-2">
            ...
            <span class="text-gray-700 dark:text-gray-300"><strong>Include twitter:card</strong> tag. Without it, Twitter assumes <code>summary</code> and crops your image into a tiny square. Use <code>summary_large_image</code>.</span>
          </li>
          <li class="flex items-start gap-2">
            <UIcon name="i-heroicons-check-circle" class="w-5 h-5 text-green-500 shrink-0 mt-0.5" />
          ...
          <li class="flex items-start gap-2">
            <UIcon name="i-heroicons-check-circle" class="w-5 h-5 text-green-500 shrink-0 mt-0.5" />
            <span class="text-gray-700 dark:text-gray-300"><strong>Keep text out of corners</strong> as Twitter rounds the corners of the preview card natively.</span>
          </li>
        </ul>
      </div>
      ...
      <div>
        ...
        <UCard class="bg-sky-50 dark:bg-sky-900/10 border-sky-100 dark:border-sky-900">
          <h3 class="font-semibold text-sky-900 dark:text-sky-300 mb-2">Twitter Card Validator Removed</h3>
          <p class="text-sm text-sky-800 dark:text-sky-200 mb-4">
            Twitter (X) officially removed their Card Validator tool. The only reliable way to check your caching and preview is to use the Post composer directly on the platform.
          </p>
        </UCard>
      </div>
    </div>
